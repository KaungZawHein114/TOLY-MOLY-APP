# TOLY MOLY — Chatbot RAG (Pinecone + OpenAI) [ARCHIVED]

> **Archived 2026-07-29** as part of the Firebase removal / "AI features become demo logic"
> refactor. This is a preserved copy of `functions/RAG.md` from the deleted Firebase Cloud
> Functions backend — kept for reference only. `knowledge/` was never actually populated (this
> doc describes the intended setup), and the RAG pipeline never went live. The chatbot's
> rule-based topic/intent gate this doc describes (`intent_service.js`) is already fully mirrored
> in `lib/core/utils/ai_mock.dart` — nothing here needs porting for that part.
